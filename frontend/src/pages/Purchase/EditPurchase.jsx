...
const EditPurchase = ({ url }) => {
  useEffect(() => {
    document.title = "Edit Purchase | Ajjawam";
  }, []);
  ...
  // Fetch existing purchase when editing
  useEffect(() => {
    const fetchPurchase = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${url}/api/purchase/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        setCompanyData({
          ...
          shortName: purchase.company?.shortName || "",
          state: purchase.company?.state || "",
          contactPhone: purchase.company?.contactPhone || "",
          gstNumber: purchase.company?.gstNumber || "",
          address: purchase.company?.address || "",
          broker: purchase.company?.broker || "",
        });
        setSelectedCompany(purchase.company);

        // fill purchase details
        setSelectedDate(new Date(purchase.date));
        setInvoiceNo(purchase.invoiceNumber || "");
        setOrderNo(purchase.orderNumber || "");
        setDiscount(purchase.discount || 0);
        setRemarks(purchase.remarks || "");
        setTransport({
          name: purchase.transportName || "",
          city: purchase.transportCity || "",
        });

        // fill products
        const prods = purchase.products.map((p) => ({
          name: p.name,
          type: p.type,
          hsn: p.hsn,
          quantity: p.quantity,
        ...
        setProducts(prods);
        setSelectedProducts(purchase.products.map((p) => p.product || null));
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /*if (company) {
          // Always update if company already exists
      ...
      await axios.put(`${url}/api/purchase/${id}`, purchaseData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <>
      <p className="bread">Edit Purchase</p>
